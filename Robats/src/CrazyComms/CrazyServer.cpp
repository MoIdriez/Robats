...
CrazyServer::CrazyServer(int port, int* data): port(port), data(data), clientsocket(0) {
		if (port < 1024) {
		...
		mainthread = thread(&CrazyServer::start, this);
		mainthread.detach();
}

void CrazyServer::start() {
